...
- (NSArray *)pupSort:(NSArray *)list {
    ...
    NSMutableArray *muta = [NSMutableArray arrayWithArray:list];

    for (int i = 0; i < muta.count; i++) {

        for (int j = 0; j < (muta.count - 1 - i); j++) {

            if ([muta[j] integerValue] > [muta[j+1] integerValue]) {

                [muta exchangeObjectAtIndex:j withObjectAtIndex:j + 1 ];
            }
        }
    }
